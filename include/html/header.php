    ...
    <form name="zappa" action="index.php" method="post" id="zappa">
      ...
      <div id="header1">
        ...
        <div id="user">
            ...
            <div id="userdate"><?php echo strftime(TZN_DATETIME_SHX,time()-TZN_TZSERVER+(new Tzn)->_getUserTZ()); ?></div>
        </div>
...
<?php
// -------------- NAV MENU ---------------------------------------------------
      $pTmpLink = (new Tzn)->concatUrl($_SESSION['linkItems'],'sProject=0');
          ...
          <ul id="nav" class="level0">               
            ...
            <li><?php echo $langMenu['view']; ?>
              <ul id="nav_projects" class="level1">
                <li class="more"><a href="<?php echo (new Tzn)->concatUrl($pTmpLink,'show=today'); ?>" accesskey="l"><?php echo $langMenu['all_projects']; ?></a>
                  <ul>
                    <li><a href="<?php echo (new Tzn)->concatUrl($pTmpLink,'show=future'); ?>" accesskey="f"><?php echo $langMenu['future_tasks']; ?></a></li>
                    <li><a href="<?php echo (new Tzn)->concatUrl($pTmpLink,'show=past'); ?>" accesskey="p"><?php echo $langMenu['past_tasks']; ?></a></li>
                    <li><a href="<?php echo (new Tzn)->concatUrl($pTmpLink,'show=all'); ?>" accesskey="a"><?php echo $langMenu['all_tasks']; ?></a></li>
                  </ul>
                </li>
              <?php
             	while ($objProj = $objUserProjectList->rNext()) {
             		$pTmpLink = (new Tzn)->concatUrl($_SESSION['linkItems'],'sProject='.$objProj->id);
//             		$pTmpLink = Tzn::concatUrl($_SESSION['linkItems'],'sProject='.$objProj->id);
              ?>
                <li class="more"><a href="<?php echo (new Tzn)->concatUrl($pTmpLink,'show=today'); ?>"><?php $objProj->p('name'); ?></a>
                  <ul class="level2">
                    <li><a href="<?php echo (new Tzn)->concatUrl($pTmpLink,'show=future')?>"><?php echo $langMenu['future_tasks']; ?></a></li>
                    <li><a href="<?php echo (new Tzn)->concatUrl($pTmpLink,'show=past')?>"><?php echo $langMenu['past_tasks']; ?></a></li>
                    <li><a href="<?php echo (new Tzn)->concatUrl($pTmpLink,'show=all')?>"><?php echo $langMenu['all_tasks']; ?></a></li>
                  </ul>
                </li>
              <?php
                }
        ...
        <div id="rightmenu">
            <a href="<?php echo (new Tzn)->concatUrl($_SESSION['linkItems'],'sUser='.$objUser->id); ?>"><?php echo $langMenu['my_tasks']; ?></a> | 
            <a href="<?php echo (new Tzn)->concatUrl($_SESSION['linkItems'],'sUser=all'); ?>"><?php echo $langMenu['all_users']; ?></a> | 
            <?php
                if ($objUser->checkLevel(1)) {
