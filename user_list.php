...
if (!$objUser->checkLevel(1)) {
    (new Tzn)->redirect('logout.php','ERROR:'.$langMessage['denied']);
}
...
if ($sCountry = (new Tzn)->getReg($_REQUEST['country'],'/^[A-Z]+$/')) {
	$objItemList->addWhere("member.countryId='".$sCountry."'");
}

if ($sState = (new Tzn)->getReg($_REQUEST['state'],'/^[A-Z]+$/')) {
		...
		?>
			<tr<?php echo $rowStyle; ?>>
				...
				<td align="center">
                    ...
                    ?>
                    <a href="<?php echo (new Tzn)->concatUrl($pLink,'mode='.$objItem->id); ?>" onclick="return confirm('<?php echo ($objItem->enabled)?$langUser['disable_confirm']:$langUser['enable_confirm']; ?>');"><img src="skins/<?php echo FRK_SKIN_FOLDER; ?>/images/b_<?php echo $imgStatus; ?>y.png" width="20" height="16" border="0" /></a>
                    <?php
                        } else {
                    ?>
                    ...
                    ?>
					<a href="<?php echo (new Tzn)->concatUrl($pLink,'delete='.$objItem->id); ?>" onclick="return confirm('<?php echo $langUser['delete_confirm']; ?>');"><img src="skins/<?php echo FRK_SKIN_FOLDER; ?>/images/b_dele.png" width="20" height="16" border="0" /></a>
                    <?php
                    } else {
                    ?>
