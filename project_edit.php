...
if ($_REQUEST['id']) {
	...
	if (!$objEditItem->load($objUser->id,false)) {
		(new Tzn)->redirect('project_list.php','ERROR:'.$langMessage['not_found_or_denied'].' (error #424)');
		...
		if ($objMemberProject->loadPosition($objEditItem->id,$objUser->id))
        {
            ...
            (new Tzn)->redirect('project_list.php','ERROR:'.$langMessage['not_found_or_denied']);
        }
	}
...
if ($pUserCanManage) {
    ...
    if ($pMessageEditStatus) {
        (new Tzn)->redirect('project_edit.php?id='.$objEditItem->id,$pMessageEditStatus);
    }
...
if ($pMessageStatus) {
    (new Tzn)->redirect('project_list.php',$pMessageStatus);
}
