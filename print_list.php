...
if ($pProject) {
    ...
    $pLink=(new Tzn)->concatUrl($pLink,'sProject='.$pProject);
...
if ($pUser = intval($_REQUEST['sUser'])) {
    ...
    $pLink=(new Tzn)->concatUrl($pLink,'sUser='.$pUser);
...
if ($_REQUEST['sContext']) {
    ...
    $pLink=(new Tzn)->concatUrl($pLink,'sContext='.$pContext);
}

$sqlFilter = '';
$pShow = ($_REQUEST['show'])?$_REQUEST['show']:'today';
$pLink=(new Tzn)->concatUrl($pLink,'show='.$pShow);
...
$pLink=(new Tzn)->concatUrl($pLink,'sort='.$pSort);
$pLink=(new Tzn)->concatUrl($pLink,'dir='.$pDir);
